...
test.describe('Chat Persistence and Data Management', () => {
  ...
  test('メッセージが複数存在できる', async ({ page }) => {
    ...
    // 全メッセージが表示されていることを確認
    for (let i = 1; i <= 3; i++) {
      await expect(page.locator('.message.user .message-content').filter({ hasText: `メッセージ ${i}` })).toBeVisible();
    }
  });
  ...
  test('チャットでユーザーメッセージが正しく表示される', async ({ page }) => {
    ...
    await expect(page.locator('.message.user .message-content').filter({ hasText: testMessage })).toBeVisible();
  });
  ...
  test('複数のメッセージタイプを混在できる', async ({ page }) => {
    ...
    // 全メッセージが表示されていることを確認
    for (const msg of messages) {
      await expect(page.locator('.message.user .message-content').filter({ hasText: msg })).toBeVisible();
    }
